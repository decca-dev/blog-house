...
exports.registerHandle = async (req, res) => {
    ...
    if (errors.length > 0) {
        res.render('register', {
            ...
            password,
            password2,
            title: "BlogHouse", description: "Register an account with BlogHouse", route: "/users/register"
        })
    }else {
        ...
        .then(async user => {
            if (user) {
                ...
                transporter.sendMail(mailOptions, (error, info) => {
                    if (error) {
                        console.log(error);
                        req.flash('error_msg', 'Something went wrong while trying to send verification email. Please try again.')
                        res.render('register', {
                            errors,
                            name,
                            email,
                        ...
                        req.flash('success_msg', 'Activation link sent. Please click on that link to log in.');
...
exports.forgotPassword = async (req, res) => {
    ...
    if (errors.length > 0) {
        res.render('forgot', {
            errors,
            email,
            title: "BlogHouse", description: "Password recovery", route: "/users/forgot"
        })
    }else {
        User.findOne({ email: email })
        .then(async user => {
            if (!user) {
                ...
                const token = jwt.sign({ _id: user._id }, JWT_RESET, { expiresIn: '30m' });
                const CLIENT_URL = `http://${req.headers.host}`
                ...
                User.updateOne({ resetLink: token }, (err, success) => {
                    if (err) {
                        ...
                        transporter.sendMail(mailOptions, (error, info) => {
                            if (error) {
                                console.log(error);
                                req.flash('error_msg', 'Something went wrong while trying to send password reset email. Please try again.')
                                res.render('forgot', {
                                    errors,
                                    email,
                                    title: "BlogHouse", description: "Password recovery", route: "/users/forgot"
                                ...
                                req.flash('success_msg', 'Activation link sent. Please follow the instructions.');
                                res.redirect('/users/login')
                            }
                        })
                    }
        ...
        jwt.verify(token, JWT_RESET, (err, decodedToken) => {
            if (err) {
                ...
                User.findById(_id, (err, user) => {
                    if (err) {
                        req.flash('error_msg', "couldn't find any users with that email adress.")
                        res.redirect('/users/login')
                    }else {
